...
public:
    ...
    void Render() override {
        ...
        // bind the particle buffer as SSBO for instance's positions
        GLuint bindingPoint = 2; // 0 has been used by the camera UBO, 1 has been used by the DestructiveSystemUBO, so we use 2 here
        glBindBufferBase(GL_SHADER_STORAGE_BUFFER, bindingPoint, computeComponent.lock()->getParticleBuffer()[0]);
...
public:
    DestructiveCSSceneObject(std::shared_ptr<GModel> _targetMesh , RenderContext * _context = nullptr) : RenderableSceneObject() {
        ...
        this->renderComponent = std::make_shared<DestructiveRenderComponent>(_context);
    ...
    void attachToSceneComputeList(std::vector<std::shared_ptr<ComputeComponent>>& computeQueue) override {
    ...
    void detachFromSceneComputeList(std::vector<std::shared_ptr<ComputeComponent>>& computeQueue) override {
        if (computeComponent == nullptr) {
...
protected:
    // it has some compute shaders which should be excuted in an order
    // since it is openGL, we just use one CS component but with multiple shaders and execute them in order
